...
class EveningEditActivity : AppCompatActivity(){
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        eveningData = database.getEveningByDate(dateKey)
        if(eveningData == null){
            eveningData = EveningData("",0,0,0,0,0, mutableListOf(), "--- ", 0)
            eveningData!!.date = dateKey
            eveningData!!.dateInMillis = dateInMillis
        }
        ...
        next.setOnClickListener({
            when(currentStep){
                ...
                stepView.stepCount - 1 -> {
                    ...
                    database.upsertEveningData(eveningData!!)
                    Log.d(TAG, eveningData.toString())
    ...
    fun displayCurrentFragment(currentStep: Int){
        when(currentStep){
            0 -> showFragment(DailyPerformanceFragment())
            1 -> showFragment(DailySleepFragment())
            2 -> showFragment(DailyAlcoholFragment())
            3 -> showFragment(EveningConditionFragment())
        }
    }
